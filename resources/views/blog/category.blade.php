...
<div class="container mx-auto px-5 lg:max-w-screen-sm">
    ...
    @foreach ($posts as $post)
        <a class="no-underline transition block border border-lighter w-full mb-10 p-5 rounded post-card" href="{{ route('blog.show', $post->slug) }}">
            <div class="block h-post-card-image bg-cover bg-center bg-no-repeat w-full h-48 mb-5" style="background-image: url('{{ asset('img/blogmain/image-for-blog-1.jpeg') }}')"></div>
    ...
    <div class="uppercase flex items-center justify-center flex-1 py-5 font-sans">        
        <a href="https://blog.laravel.com?page=2" rel="next" class="block no-underline text-light hover:text-black px-5">Check More Articles</a>
    </div>
</div>
